            ...
            <div class="card my-4">
                <div class="card-header p-0 position-relative mt-n4 mx-3 z-index-2">
                    <div class="bg-gradient-primary shadow-primary border-radius-lg pt-4 pb-3 d-flex">
                        <h6 class="text-white text-capitalize ps-3">{{ $title ?? 'Đơn vị tính' }}</h6>
                        <div class="row">
                            <div class="col-md-12 " style="margin-left: 900px">
                                <a class="btn btn-sm btn-info" href="{{ route('unit.create') }}">
                                    <i class="fas fa-save"></i> Thêm
                                </a>
                                {{-- <a class="btn btn-sm btn-danger" href="{{ route('unit.trash') }}">
                            ...
                            <tbody>
                                ...
                                @foreach ($list as $unit)
                                    <tr class="text-center">
                                        <td>
                                            <input type="checkbox" name="checkId[]" value="{{ $unit->id }}">
                                        </td>

                                        <td>
                                            <p class="text-xs font-weight-bold mb-0 text-center"> {{ $unit->unitcode }}</p>

                                        </td>
                                        <td>
                                            <p class="text-xs font-weight-bold mb-0 text-center"> {{ $unit->name }}</p>
                                        ...
                                        <td class="text-center ">
                                            <div
                                                ...
                                                @if ($unit->status == 1)
                                                    <a href="{{ route('unit.status', ['unit' => $unit->id]) }}"
                                                ...
                                                <a href="{{ route('unit.delete', ['unit' => $unit->id]) }}"
                                                    class="btn btn-sm btn-danger">
